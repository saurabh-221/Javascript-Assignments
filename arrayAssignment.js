...
function fun1()
{
  ...
  const a=inventory[32]["car_year"]
  const b=inventory[32]["car_make"]
  const c=inventory[32]["car_model"]
  console.log(`Car 33 is a ${a} ${b} ${c}` );
}  
...
function fun2()
{
  ...
  let lastCar = 49;
  const a=inventory[49]["car_make"]
  const b=inventory[49]["car_model"]
  console.log(`The make and model of last car in the inventory is ${a} and ${b}`);
}
...
function fun6()
{
  ...
  for(let i=0;i<inventory.length;i++)
  {
      if (inventory[i]["car_make"]==="BMW" || inventory[i]["car_make"]==="Mercedes-Benz")
      {
          a.push(inventory[i])
      }
  }
